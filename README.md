...
3. **Segmentation** — splits the population into Thin File vs. Traditional and compares default rates and feature profiles (`3_borrower_segmentation/`).
4. **Modelling** — trains two separate XGBoost classifiers (one per segment) so the traditional-credit model doesn't penalize thin-file borrowers for missing fields, and the thin-file model isn't diluted by traditional-only features (`4_predictive_modelling/`).
...
7. **Dashboard** — interactive Streamlit/Plotly app exposing portfolio KPIs, default-rate breakdowns, and risk-tier drill-downs (`7_streamlit_dashboard/`).
8. **Business insights** — executive summary translating model output into a lending-policy recommendation (`8_business_insights/`).

## Tech Stack

Python (pandas, NumPy, scikit-learn, XGBoost) · SQLite/SQL · Streamlit · Plotly
...
# 3. Segment thin-file vs. traditional borrowers
python 3_borrower_segmentation/segmentation_analysis.py

# 4. Train risk models and score borrowers
